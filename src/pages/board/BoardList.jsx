      ...
      try {
        ...
        const filterCategory =
          selectedCategory === "all"
            ? rsp.data
            : rsp.data.filter(
                (el) => el.categoryId === parseInt(selectedCategory)
              );
        setBoardList(filterCategory);
      } catch (e) {
        console.error(e);
      }
    };
        ...
        {boardList &&
          boardList.map((el, idx) => (
            <BoardLi
              key={idx}
              onClick={() => {
                handleDetailClick(el.boardId);
              }}
            >
              <BoardImage
                src={el.img ? el.img : "http://via.placeholder.com/160"}
                alt="Board Img"
              />
              <BoardContentWrapper>
                <BoardHeader>
                  <BoardTitle>{el.title}</BoardTitle>
                  <UserId>{el.email}</UserId>
                </BoardHeader>
                <BoardContent>{el.content}</BoardContent>
                <BoardDate>{Common.timeFromNow(el.regDate)}</BoardDate>
              </BoardContentWrapper>
            </BoardLi>
          ))}
      </BoardUl>
